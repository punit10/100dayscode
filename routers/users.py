...
# main.py "/api/users" and users.py "/{user_id}" adds to => "/api/users/{user_id}"
...
# Checking name and email case in-sensitive
...
## forgot_password endpoint, called when user clicks to forgot password, & sent reset link
...
## reset_password endpoint, called when user clicks the link sent via email
...
async def get_user_posts(
    ...
    has_more_posts = skip + len(posts) < total
...
async def update_user(
    ...
    # manual way for model.dump() and setattr(post, field, value),
    # in production best practice use setattr()
    if user_update.username is not None:
...
async def delete_user(user_id: int,
    ...
    # If user deleted then delete profile file image
    if old_filename:
