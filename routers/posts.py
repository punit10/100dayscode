...
async def get_posts(db: Annotated[AsyncSession, Depends(get_db)],
    ...
    has_more_posts = skip + len(posts) < total_post_count
...
async def update_post_full(
    ...
    if post.user_id != current_user.id:
        raise HTTPException(
            ...
            detail="Not authorized to edit the post!",
        )
    # if post and user checks passed, update the post with new details
    post.title = post_data.title
    post.content = post_data.content
    # post.user_id = current_user.id # not needed now as only logged-in user can edit
